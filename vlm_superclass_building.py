...
def process_labels(im_labels, hierarchy):
    count = len(im_labels)
    ret_labels = []
...
if __name__ == "__main__":
    ...
    print("Creating superclass associations with VLM...")

    #parse input
    parser = argparse.ArgumentParser(description='Creating superclass associations with VLM.')
    ...
    parser.add_argument('--root_imagenet', type=str, default='/media/data/Datasets/imagenet21k_resized', help='root path to train set of imagenet')
    parser.add_argument('--log_file', type=str, default='vlm_superclass_building.txt', help='log file')

    args = parser.parse_args()

    log_folder = "logs"
    os.makedirs(log_folder, exist_ok=True)
    log_file = os.path.join(log_folder, args.log_file)
    #clear previous log file
    with open(log_file, 'w') as file:
        file.write("")

    # base imagent path
    base_imagenet_path_train = os.path.join(args.root_imagenet, 'imagenet21k_train')
    base_imagenet_path_val = os.path.join(args.root_imagenet, 'imagenet21k_val')

    #### PROCESS LISTS

    imagent_mapping_file = './data_class_lists/imagenet_cls.yaml'
    if not os.path.exists(imagent_mapping_file):
        raise FileNotFoundError(
            f"File {input_file} not found, please check that the file with imagenet synsets and class names is in the correct path.")

    selected_classes_file = './data_class_lists/selected_classes.yaml'
    if not os.path.exists(selected_classes_file):
        raise FileNotFoundError(
            f"File {selected_classes_file} not found, please check that the file with user's selected classes is in the correct path.")

    # Carica i file YAML
    with open(selected_classes_file, 'r') as file:
        list1 = yaml.safe_load(file)

    #reverse list1 dict
    list1_reverse = {v: k for k, v in list1.items()}

    with open(imagent_mapping_file, 'r') as file:
    ...
    res_dict = {user_class : [] for user_class in class_names_list1}
    ...
    if not os.path.exists(out_file):
        for user_class in tqdm(class_names_list1, desc="Pre-processing classes..."):

            #search directly in the hierarchy
            im_labels = hierarchy.get(user_class)

            if im_labels: #found a direct match
                res_dict[user_class].append(user_class)
                for im_label in im_labels:
                    res_dict[user_class].append(im_label)

                ret_labels = process_labels(im_labels, hierarchy) #add children of first level children
                for im_label in ret_labels:
                    res_dict[user_class].append(im_label)
            else:
                #find the closest match
                user_class_spaced = user_class.replace("_", " ")
                im_label_spaced_list = findBestMatch(user_class, class_names_list2_spaced)
                if im_label_spaced_list:
                    ...
                    for (im_label_spaced, score) in im_label_spaced_list:
                        ...
                        res_dict[user_class].append(im_label)
                    ret_labels = process_labels(im_labels, hierarchy)
                    for im_label in ret_labels:
                        res_dict[user_class].append(im_label)

                res_dict[user_class] = list(set(res_dict[user_class])) #remove replicas
        ...
        for key in tqdm(res_dict.keys(), desc="Processing with VLM..."):
                    ...
                    if os.path.exists(folder):
                        ...
                        if not os.path.exists(folder):
                            ...
                            with open(log_file, 'a') as file:
                                file.write(f"Folder {folder} not found, skipping...\n")
                            continue
                        folder_paths.append(folder)
                        folders.append(folder)
                        classes.append(im_label)
        ...
        with open(out_file, 'r') as file:
            ...
            print(f"Loaded out_dict from {out_file}")
